export function StatCard({ label, value, unit, icon: Icon, color = "blue", sub, delta }) {
  const colors = {
  ...
  const iconColors = {
    ...
    purple: "text-purple-500", red: "text-red-500", teal: "text-teal-500",
  };
  ...
  return (
    <div className={`rounded-2xl border p-5 flex flex-col gap-1 ${colors[color]}`}>
      ...
      {sub && delta == null && <span className="text-xs opacity-60">{sub}</span>}
    </div>
  );
}
